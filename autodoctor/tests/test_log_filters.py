...
def _incident(pattern: str = "kasa/authentication/a8f6419a8c") -> logging.LogRecord:
    return logging.LogRecord(
        name="autodoctor.engine",
        level=logging.INFO,
        pathname=__file__,
        lineno=1,
        msg=KasaIncidentCoalescingFilter.PREFIX,
        args=("fingerprint", pattern, 41251, "kasa.component", "poll failed"),
        exc_info=None,
    )
...
def test_kasa_incident_logs_first_then_periodic_summary() -> None:
    ...
    assert filter_.filter(second) is False
    assert filter_.filter(third) is True
    assert "coalesced_kasa_events=%s" in str(third.msg)
    assert third.args[-1] == 3


def test_non_kasa_incident_logs_are_untouched() -> None:
    filter_ = KasaIncidentCoalescingFilter(every=3)
    record = _incident("aiohttp/timeout/abc")
